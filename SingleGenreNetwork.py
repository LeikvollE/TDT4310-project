...
class SingleLyricSTM(nn.Module):
...
if __name__ == "__main__":
    minibatch_size = 64
    model_file = "models/newnetonegenre2/model"
    ...
    model = SingleLyricSTM(256, len(alphabet), alphabet, unique_genres)
    model_ = torch.load("models/newnetonegenre2/model_checkpoint_6")
    ...
    for i in range(7, n_steps):
        ...
        for j in tqdm(range(0, len(songs), minibatch_size)):
            ...
            batch_encoded, batch_targets, genres_encoded = encode(batch_songs, alphabet, batch_genres, unique_genres)
            X = batch_encoded[:-1]
            Y = batch_targets[1:]
            Y = Y.long()
            Y = Y.permute(1, 0)
            ...
            def closure():
                optimizer.zero_grad()
                out = model(X, genres_encoded, temp=0.45)
